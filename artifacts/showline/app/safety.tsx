...
export default function SafetyScreen() {
  ...
  const handleUnblock = (id: string, name: string) => {
    Alert.alert("Unblock Contact", `Allow ${name} to message you again?`, [
      ...
      { text: "Unblock", onPress: () => removeBlocked(id) },
    ]);
  };
  ...
  return (
    ...
    >
      ...
      <View style={styles.section}>
        <Text style={[styles.sectionTitle, { color: colors.mutedForeground }]}>BLOCKED CONTACTS</Text>
          ...
          blockedContacts.map((b) => (
            <View key={b.id} style={[styles.blockedCard, { backgroundColor: colors.card }]}>
              ...
              <View style={styles.blockedInfo}>
                ...
                {b.handle && (
                  <Text style={[styles.blockedHandle, { color: colors.mutedForeground }]}>{b.handle}</Text>
                )}
                {b.reason && (
                  <Text style={[styles.blockedReason, { color: colors.mutedForeground }]}>{b.reason}</Text>
                )}
                <Text style={[styles.blockedAt, { color: colors.border }]}>Blocked {b.blockedAt}</Text>
              </View>
              <Pressable
      ...
      {/* Usage policies */}
      <View style={styles.section}>
        <Text style={[styles.sectionTitle, { color: colors.mutedForeground }]}>USAGE POLICIES</Text>
        {POLICIES.map((p) => (
      ...
      <View style={[styles.reportBox, { backgroundColor: "#EF444411", borderColor: "#EF444433" }]}>
        ...
        <View style={styles.reportText}>
          ...
          <Text style={[styles.reportDesc, { color: colors.mutedForeground }]}>
            If you believe someone is misusing ShowLine, you can report them from any message using the
            Block action. Severe cases can be escalated to our trust & safety team.
          </Text>
        </View>
      </View>
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  shieldWrap: { width: 56, height: 56, borderRadius: 16, alignItems: "center", justifyContent: "center" },
  ...
  emptyBlock: { flexDirection: "row", alignItems: "center", gap: 12, padding: 16, borderRadius: 14 },
  emptyBlockText: { fontSize: 14, fontFamily: "Inter_500Medium" },
  blockedCard: { flexDirection: "row", alignItems: "center", padding: 14, borderRadius: 14, gap: 12 },
  blockedAvatar: { width: 40, height: 40, borderRadius: 12, alignItems: "center", justifyContent: "center" },
  ...
  policyCard: { flexDirection: "row", gap: 12, padding: 14, borderRadius: 14, alignItems: "flex-start" },
  ...
  reportBox: { flexDirection: "row", gap: 12, padding: 14, borderRadius: 14, borderWidth: 1, alignItems: "flex-start" },
