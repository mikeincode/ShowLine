...
const TEMPLATE_INFO: Record<keyof AutoReplies, { label: string; desc: string; color: string; icon: string }> = {
  fanmail: { label: "FanMail", desc: "Sent when fans message your general line", color: "#8B5CF6", icon: "mail" },
  livelineOpen: { label: "LiveLine (Open)", desc: "Sent during an active live session", color: "#EF4444", icon: "radio" },
  livelineClosed: { label: "LiveLine (Closed)", desc: "Sent when live session is inactive", color: "#6B7280", icon: "radio" },
  backstage: { label: "Backstage Line", desc: "Sent to VIP superfans", color: "#F59E0B", icon: "star" },
  collab: { label: "Collab Line", desc: "Sent to business inquiry senders", color: "#3B82F6", icon: "briefcase" },
};

const KEYS: (keyof AutoReplies)[] = ["fanmail", "livelineOpen", "livelineClosed", "backstage", "collab"];
    ...
    >
      ...
      {KEYS.map((key) => {
        ...
        return (
          <View
            key={key}
            style={[styles.templateCard, { backgroundColor: colors.card, borderColor: isEditing ? info.color + "88" : colors.border, borderWidth: isEditing ? 1.5 : 0 }]}
          >
            <View style={styles.templateHeader}>
              ...
              <View style={styles.templateMeta}>
                <Text style={[styles.templateLabel, { color: colors.foreground }]}>{info.label}</Text>
                <Text style={[styles.templateDesc, { color: colors.mutedForeground }]}>{info.desc}</Text>
              </View>
              {!isEditing && (
                <Pressable
            ...
            {isEditing ? (
              <>
                <TextInput
                  ...
                  style={[styles.editor, { color: colors.foreground, backgroundColor: colors.muted, borderColor: info.color + "44" }]}
                  multiline
                  autoFocus
                ...
                <View style={styles.editActions}>
                  <Pressable onPress={cancelEdit} style={[styles.cancelBtn, { borderColor: colors.border }]}>
...
const styles = StyleSheet.create({
  ...
  cancelBtn: { flex: 1, height: 42, borderRadius: 10, alignItems: "center", justifyContent: "center", borderWidth: 1 },
  cancelText: { fontSize: 14, fontFamily: "Inter_600SemiBold" },
  saveBtn: { flex: 1, height: 42, borderRadius: 10, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6 },
